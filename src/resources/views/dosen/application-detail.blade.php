...
@section('content')
        ...
        <div class="assignment-card">
            <div class="assignment-header">
                <div class="assignment-info">
                    ...
                    <div class="assignment-meta ms-auto">
            ...
            <div class="assignment-body">
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <p><strong>Tujuan Perusahaan / Instansi : </strong> {{ $application->institution_name }}</p>
                                <p><strong>Bidang Usaha : </strong>
                                    {{ $application->businessField ? $application->businessField->name : $application->business_field ?? 'N/A' }}
                                </p>
                                <p><strong>Leader KP : </strong> {{ $application->submittedBy->name }}</p>
                            </div>
                            <div class="col-md-6">
                                <p><strong>Tanggal Permohonan : </strong>
                                    {{ $application->created_at->format('M d, Y H:i') }}
                                </p>
                                <p><strong>Tanggal KP : </strong> {{ $application->planned_start_date->format('M d, Y') }}
                                    - {{ $application->planned_end_date->format('M d, Y') }}</p>
                                @if ($application->reviewed_by)
                                    <p><strong>Dosen Pembimbing : </strong> {{ $application->reviewedBy->name }}</p>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="text-center">
                            @if (
                                ...
                                <img src="{{ $application->student->user->identity->image_url }}" alt="Student"
                                    class="img-fluid rounded-circle mb-3" style="max-width: 120px;">
                            @else
                                <div class="bg-light rounded-circle d-inline-flex align-items-center justify-content-center mb-3"
                                    style="width: 120px; height: 120px;">
                                    <span
                                        class="text-muted fs-1">{{ $application->student ? substr($application->student->nama_resmi, 0, 2) : 'N/A' }}</span>
                                </div>
                            @endif
                            <h5>{{ $application->student ? $application->student->nama_resmi : 'N/A' }}</h5>
                            <p class="text-muted">{{ $application->student ? $application->student->email_kampus : 'N/A' }}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="assignment-body">
                <div class="action-buttons">
                    ...
                    @if ($application->status == 'submitted')
                        <button type="button" class="btn btn-primary choose-reviewer-btn"
                            data-application-id="{{ $application->id }}">
                            <i class="fas fa-user-check"></i> Choose Reviewer
                        </button>
                    @endif
                    @if ($application->status == 'reviewing' && $application->reviewed_by == Auth::id())
                        <form method="POST" action="{{ route('dosen.applications.approve', $application->id) }}"
                            class="d-inline">
                            @csrf
                            @method('PATCH')
                        ...
                        <form method="POST" action="{{ route('dosen.applications.reject', $application->id) }}"
                            class="d-inline">
                            @csrf
                            @method('PATCH')
                    ...
                    @endif
                    <a href="{{ route('dosen.applications.view', $application->id) }}" class="btn btn-primary">
                        <i class="fas fa-eye"></i> View Aplikasi Permohonan
                    </a>
    ...
    @if ($application->status == 'submitted')
        <div class="modal fade" id="chooseReviewerModal" tabindex="-1" aria-labelledby="chooseReviewerModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" id="assignReviewerBtn" disabled>Assign
                            Reviewer</button>
                    </div>
                </div>
            </div>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function displayReviewers(reviewers) {
                    ...
                    if (reviewers.length === 0) {
                        reviewersList.innerHTML =
                            '<div class="col-12"><p class="text-center text-muted">No reviewers available</p></div>';
                        return;
                    }
